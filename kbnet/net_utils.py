...
class Conv2d(torch.nn.Module):
    ...
    def __init__(
        ...
        if weight_initializer == "kaiming_normal":
            ...
            raise ValueError(
                "Unsupported weight initializer: {}".format(weight_initializer)
            )

        self.activation_func = activation_func

        assert not (
            use_batch_norm and use_instance_norm
...
class AtrousSpatialPyramidPooling(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        global_pool = torch.nn.functional.interpolate(
            global_pool, size=x.shape[2:], mode="bilinear", align_corners=True
        )
...
def pose_matrix(v, rotation_parameterization="axis"):
    ...
    if rotation_parameterization == "axis":
        ...
        raise ValueError(
            "Unsupported rotation parameterization: {}".format(
...
def meshgrid(n_batch, n_height, n_width, device, homogeneous=True):
    ...
    grid_xy = torch.unsqueeze(grid_xy, dim=0).repeat(n_batch, 1, 1, 1)
...
def project_to_pixel(points, pose, intrinsics, shape):
    ...
    row = (
        torch.tensor([0.0, 0.0, 0.0, 1.0], device=points.device)
        .view(1, 1, 4)
        .repeat(n_batch, 1, 1)
    )
