...
class Transforms(object):
    def __init__(
        ...
        self.do_random_horizontal_flip = (
            True if "horizontal" in random_flip_type else False
        )
    ...
    def transform(
        ...
        images_arr = self.normalize_images(
            images_arr, normalized_image_range=self.normalized_image_range
        )

        if self.do_random_horizontal_flip:
            do_horizontal_flip = np.logical_and(
                do_random_transform, np.random.rand(n_batch) <= 0.50
            )
            ...
            validity_maps_arr = self.horizontal_flip(
                validity_maps_arr, do_horizontal_flip
            )
        ...
        if self.do_random_remove_points:
            ...
            densities = (
                remove_points_max - remove_points_min
            ) * values + remove_points_min
    ...
    def normalize_images(self, images_arr, normalized_image_range=[0, 1]):
        ...
        if normalized_image_range == [0, 1]:
            images_arr = [images / 255.0 for images in images_arr]
        elif normalized_image_range == [-1, 1]:
            images_arr = [2.0 * (images / 255.0) - 1.0 for images in images_arr]
        elif normalized_image_range == [0, 255]:
            pass
        else:
            raise ValueError(
                "Unsupported normalization range: {}".format(normalized_image_range)
            )
    ...
    def random_nonzero(self, T, density=0.10):
        ...
        random_nonzero_indices = [indices[random_subset] for indices in nonzero_indices]
            ...
            for b, image in enumerate(images):
                if do_add_noise[b]:
                    ...
                    validity_map = torch.where(
                        image > 0, torch.ones_like(image), torch.zeros_like(image)
                    )

                    if noise_type == "gaussian":
                        image = image + noise_spread * torch.randn(
                            *shape, device=device
                        )
                    elif noise_type == "uniform":
                        image = image + noise_spread * (
                            torch.rand(*shape, device=device) - 0.5
                        )
                    else:
                        raise ValueError(
                            "Unsupported noise type: {}".format(noise_type)
                        )
