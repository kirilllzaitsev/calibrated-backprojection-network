...
class KBNetModel(object):
    ...
    def __init__(
        ...
        n_filters_encoder = [
            i + z for i, z in zip(n_filters_encoder_image, n_filters_encoder_depth)
        ]
    ...
    def forward(self, image, sparse_depth, validity_map_depth, intrinsics):
        """
        ...
        input_depth = [sparse_depth, validity_map_depth]
        ...
        output_depth = self.min_predict_depth / (
            output_depth + self.min_predict_depth / self.max_predict_depth
        )
    ...
    def compute_loss(
        ...
        loss_smoothness = losses.smoothness_loss_func(
            predict=output_depth0, image=image0
        )
    ...
    def save_model(self, checkpoint_path, step, optimizer):
        ...
        checkpoint[
            "sparse_to_dense_pool_state_dict"
        ] = self.sparse_to_dense_pool.state_dict()
    ...
    def restore_model(self, checkpoint_path, optimizer=None):
        ...
        self.sparse_to_dense_pool.load_state_dict(
            checkpoint["sparse_to_dense_pool_state_dict"]
        )
        ...
        with torch.no_grad():
                ...
                display_summary_image.append(
                    torch.cat(
                        [
                            image0_summary.cpu(),
                            torch.zeros_like(
                                image0_summary, device=torch.device("cpu")
                            ),
                        ],
                        dim=-1,
                    )
            ...
            if image0 is not None and image01 is not None:
                ...
                image01_error_summary = torch.mean(
                    torch.abs(image0_summary - image01_summary), dim=1, keepdim=True
                )

                # Add to list of images to log
                image01_error_summary = log_utils.colorize(
                    (image01_error_summary / 0.10).cpu(), colormap="inferno"
                )

                display_summary_image.append(
                    torch.cat([image01_summary.cpu(), image01_error_summary], dim=3)
                )
            ...
            if image0 is not None and image02 is not None:
                ...
                image02_error_summary = torch.mean(
                    torch.abs(image0_summary - image02_summary), dim=1, keepdim=True
                )

                # Add to list of images to log
                image02_error_summary = log_utils.colorize(
                    (image02_error_summary / 0.10).cpu(), colormap="inferno"
                )

                display_summary_image.append(
                    torch.cat([image02_summary.cpu(), image02_error_summary], dim=3)
                )
            ...
            if output_depth0 is not None:
                ...
                summary_writer.add_histogram(
                    tag + "_output_depth0_distro", output_depth0, global_step=step
                )

            if (
                output_depth0 is not None
                and sparse_depth0 is not None
                and validity_map0 is not None
            ):
                ...
                sparse_depth0_error_summary = torch.abs(
                    output_depth0_summary - sparse_depth0_summary
                )

                sparse_depth0_error_summary = torch.where(
                    validity_map0_summary == 1.0,
                    (sparse_depth0_error_summary + EPSILON)
                    / (sparse_depth0_summary + EPSILON),
                ...
                sparse_depth0_error_summary = log_utils.colorize(
                    (sparse_depth0_error_summary / 0.05).cpu(), colormap="inferno"
                )

                display_summary_depth.append(
                    torch.cat(
                        [sparse_depth0_summary, sparse_depth0_error_summary], dim=3
                    )
                )

                # Log distribution of sparse depth
                summary_writer.add_histogram(
                    tag + "_sparse_depth0_distro", sparse_depth0, global_step=step
                )
            ...
            if output_depth0 is not None and ground_truth0 is not None:
                ...
                ground_truth0_error_summary = torch.abs(
                    output_depth0_summary - ground_truth0_summary
                )

                ground_truth0_error_summary = torch.where(
                    validity_map0_summary == 1.0,
                    (ground_truth0_error_summary + EPSILON)
                    / (ground_truth0_summary + EPSILON),
                    validity_map0_summary,
                )
                ...
                ground_truth0_error_summary = log_utils.colorize(
                    (ground_truth0_error_summary / 0.05).cpu(), colormap="inferno"
                )

                display_summary_depth.append(
                    torch.cat(
                        [ground_truth0_summary, ground_truth0_error_summary], dim=3
                    )
                )

                # Log distribution of ground truth
                summary_writer.add_histogram(
                    tag + "_ground_truth0_distro", ground_truth0, global_step=step
                )

            if pose01 is not None:
                # Log distribution of pose 1 to 0translation vector
                summary_writer.add_histogram(
                    tag + "_tx01_distro", pose01[:, 0, 3], global_step=step
                )
                summary_writer.add_histogram(
                    tag + "_ty01_distro", pose01[:, 1, 3], global_step=step
                )
                summary_writer.add_histogram(
                    tag + "_tz01_distro", pose01[:, 2, 3], global_step=step
                )

            if pose02 is not None:
                # Log distribution of pose 2 to 0 translation vector
                summary_writer.add_histogram(
                    tag + "_tx02_distro", pose02[:, 0, 3], global_step=step
                )
                summary_writer.add_histogram(
                    tag + "_ty02_distro", pose02[:, 1, 3], global_step=step
                )
                summary_writer.add_histogram(
                    tag + "_tz02_distro", pose02[:, 2, 3], global_step=step
                )
