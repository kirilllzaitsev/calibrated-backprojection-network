...
def train(
    ...
    n_train_step = learning_schedule[-1] * np.ceil(n_train_sample / n_batch).astype(
        np.int32
    )
    ...
    validation_available = (
        val_image_path is not None
        and val_sparse_depth_path is not None
        and val_intrinsics_path is not None
        and val_ground_truth_path is not None
    )
    ...
    if validation_available:
        ...
        val_transforms = Transforms(normalized_image_range=normalized_image_range)

    # Initialize outlier removal for sparse depth
    outlier_removal = OutlierRemoval(
        kernel_size=outlier_removal_kernel_size, threshold=outlier_removal_threshold
    )
    ...
    for epoch in range(1, learning_schedule[-1] + 1):
        ...
        if (
            -1 not in augmentation_schedule
            and epoch > augmentation_schedule[augmentation_schedule_pos]
        ):
            augmentation_schedule_pos = augmentation_schedule_pos + 1
            augmentation_probability = augmentation_probabilities[
...
def validate(
    ...
    for idx, (inputs, ground_truth) in enumerate(zip(dataloader, ground_truths)):
        ...
        min_max_mask = np.logical_and(
            ground_truth > min_evaluate_depth, ground_truth < max_evaluate_depth
        )
    ...
    log(
        "{:8}  {:8.3f}  {:8.3f}  {:8.3f}  {:8.3f}".format(step, mae, rmse, imae, irmse),
        log_path,
    )
...
def run(
    ...
    input_paths = [image_paths, sparse_depth_paths, intrinsics_paths]
    ...
    transforms = Transforms(normalized_image_range=normalized_image_range)
    ...
    if ground_truth_available:
        ...
        log(
            "{:>8}  {:>8}  {:>8}  {:>8}".format("MAE", "RMSE", "iMAE", "iRMSE"),
            log_path,
        )
        log(
    ...
    # Log run time
    log(
        "Total time: {:.2f} ms  Average time per sample: {:.2f} ms".format(
    ...
    if save_outputs:
        ...
        for idx, (image, output_depth, sparse_depth, ground_truth) in enumerate(
            outputs
        ):
...
def log_input_settings(
    ...
    batch_settings_text = (
        batch_settings_text + "  "
        if len(batch_settings_text) > 0
        else batch_settings_text
    )
    ...
    if len(batch_settings_vars) > 0:
        log(batch_settings_text.format(*batch_settings_vars), log_path)

    log(
        "input_channels_image={}  input_channels_depth={}".format(
    ...
    log(
        "outlier_removal_kernel_size={}  outlier_removal_threshold={:.2f}".format(
...
def log_network_settings(
    ...
    log(
        "min_pool_sizes_sparse_to_dense_pool={}".format(
            min_pool_sizes_sparse_to_dense_pool
        ),
        log_path,
    )
    log(
        "max_pool_sizes_sparse_to_dense_pool={}".format(
            max_pool_sizes_sparse_to_dense_pool
        ),
        log_path,
    )
    log(
        "n_convolution_sparse_to_dense_pool={}".format(
            n_convolution_sparse_to_dense_pool
        ),
        log_path,
    )
    log(
        "n_filter_sparse_to_dense_pool={}".format(n_filter_sparse_to_dense_pool),
        log_path,
    )
    log("", log_path)

    log("Depth network settings:", log_path)
    log("n_filters_encoder_image={}".format(n_filters_encoder_image), log_path)
    log("n_filters_encoder_depth={}".format(n_filters_encoder_depth), log_path)
    log("resolutions_backprojection={}".format(resolutions_backprojection), log_path)
    log("n_filters_decoder={}".format(n_filters_decoder), log_path)
    log("deconv_type={}".format(deconv_type), log_path)
    log(
        "min_predict_depth={:.2f}  max_predict_depth={:.2f}".format(
    ...
    log(
        "n_parameter={}  n_parameter_depth={}  n_parameter_pose={}".format(
            n_parameter, n_parameter_depth, n_parameter_pose
        ),
        log_path,
    )
    log(
        "weight_initializer={}  activation_func={}".format(
...
def log_training_settings(
    ...
    log(
        "n_sample={}  n_epoch={}  n_step={}".format(
    ...
    log(
        ...
        % ", ".join(
            ...
            for ls, le, v in zip(
                [0] + learning_schedule[:-1], learning_schedule, learning_rates
            )
        ),
        log_path,
    )
    ...
    log(
        "augmentation_schedule=[%s]"
        % ", ".join(
            "{}-{} : {}".format(
                ls * (n_train_sample // n_batch), le * (n_train_sample // n_batch), v
            )
            for ls, le, v in zip(
                [0] + augmentation_schedule[:-1],
                augmentation_schedule,
                augmentation_probabilities,
            )
        ),
    ...
    log(
        "augmentation_random_crop_type={}".format(augmentation_random_crop_type),
        log_path,
    )
    log(
        "augmentation_random_flip_type={}".format(augmentation_random_flip_type),
        log_path,
    )
    log(
        "augmentation_random_remove_points={}".format(
            augmentation_random_remove_points
        ),
        log_path,
    )
    log(
        "augmentation_random_noise_type={}  augmentation_random_noise_spread={}".format(
...
def log_loss_func_settings(
    ...
    log(
        "w_color={:.1e}  w_structure={:.1e}  w_sparse_depth={:.1e}".format(
            w_color, w_structure, w_sparse_depth
        ),
        log_path,
    )
    log("w_smoothness={:.1e}".format(w_smoothness), log_path)
    log(
        "w_weight_decay_depth={:.1e}  w_weight_decay_pose={:.1e}".format(
...
def log_evaluation_settings(log_path, min_evaluate_depth, max_evaluate_depth):
    log("Evaluation settings:", log_path)
    log(
        "min_evaluate_depth={:.2f}  max_evaluate_depth={:.2f}".format(
...
def log_system_settings(
    ...
    if depth_model_restore_path is not None and depth_model_restore_path != "":
        log("depth_model_restore_path={}".format(depth_model_restore_path), log_path)

    if pose_model_restore_path is not None and pose_model_restore_path != "":
        log("pose_model_restore_path={}".format(pose_model_restore_path), log_path)
